...
description: This skill should be used after completing eligible code work to produce a ready-to-paste PR Draft block (change summary, branch suggestion, title, and a description starting with "This pull request ..."). Trigger before the final response whenever the task changed Go runtime code, migrations, tests, build/Docker config, or behavior-impacting docs.
version: 1.0.0
---
...
Produce the PR-ready summary required in this repository after eligible code work is complete: a concise summary plus a PR-ready title and draft description that begins with "This pull request ...". The block should be ready to paste into a PR for `Sm3ggl3s/pkm-atlas-api`.

## When to Trigger

- Before every final response, check whether the current task changed runtime code (`cmd/`, `internal/`), database migrations (`migrations/`), tests (`*_test.go`), build/test/Docker configuration, or docs with behavior impact.
...
- Latest release tag: `LATEST_RELEASE_TAG=$(git tag -l 'v*' --sort=-v:refname | head -n1)`. For a remote-aware lookup when local tags may be stale: `git ls-remote --tags --refs origin 'v*' | awk -F/ '{print $NF}' | sort -V | tail -n1`.
- Base reference (use the branch's upstream, fallback to `origin/development`):
  - `BASE_REF=$(git rev-parse --abbrev-ref --symbolic-full-name @{upstream} 2>/dev/null || echo origin/development)`.
...
- Category signals for this repo:
  - **runtime:** `cmd/`, `internal/`
  - **migrations:** `migrations/`
  - **tests:** `*_test.go`
  - **build/config:** `go.mod`, `go.sum`, `Makefile`, `docker-compose.yml`, `Dockerfile`, `.air.toml`
...
3. Infer change type from the touched paths listed under "Category signals"; classify as **feature**, **fix**, **refactor**, or **docs-with-impact**, and flag backward-compatibility risk only when the diff changes released public APIs, external config, persisted data (including migration changes that alter or drop existing schema/data), serialized state, or wire protocols. Judge that risk against `LATEST_RELEASE_TAG`, not unreleased branch-only churn.
...
6. Suggest a branch name. Branches are cut from and PR back into `development` (`main` is the release branch). If already off `development`, keep the current branch; otherwise propose `feat/<slug>`, `fix/<slug>`, or `docs/<slug>` based on the primary area (e.g., `feat/pokemon-catalog-migrations`).
7. If the current branch matches `issue-<number>` (digits only), keep that branch suggestion. Optionally pull light issue context (for example via the GitHub API) when available, but do not block or retry if it is not. When an issue number is present, reference `https://github.com/Sm3ggl3s/pkm-atlas-api/issues/<number>` and include an auto-closing line such as `This pull request resolves #<number>.`.
...
git checkout -b <kebab-case suggestion, e.g., feat/pokemon-catalog-migrations>
...
Keep it tight—no redundant prose around the block, and avoid repeating details between the change summary and the description. Tests do not need to be listed unless specifically requested.
